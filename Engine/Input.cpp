...
Input::Input(HWND window) {
	...
	DirectInput8Create(GetModuleHandleW(NULL), DIRECTINPUT_VERSION, IID_IDirectInput8, (void**)&m_di, NULL);

	//Create, prepare and aquire the keyboard deivce
	...
	//Create, prepare and aquire the mouse deivce
...
void Input::Update() {
	...
	while (true) {
		m_mouse->Poll();
		if (SUCCEEDED(result = m_mouse->GetDeviceState(sizeof(DIMOUSESTATE), (LPVOID)&m_mouseState)))
			break;
		if (result != DIERR_INPUTLOST && result != DIERR_NOTACQUIRED)
			return;
