...
// Vector structure
struct Vertex {
	D3DXVECTOR3 translation;		// Transalation of the vertex
...
inline FLOAT CalculateDotProduct(const D3DXPLANE* plane, FLOAT x, FLOAT y, FLOAT z) {
	D3DXVECTOR3 vector(x, y, z);
	return D3DXPlaneDotCoord(plane, &vector);
}
...
inline bool IsBoxEnclosedByVolume(LinkedList< D3DXPLANE >* planes, D3DXVECTOR3 min, D3DXVECTOR3 max)
{
	...
	{
		if (CalculateDotProduct(planes->GetCurrent(), min.x, min.y, min.z) < 0.0f)
			return false;
		if (CalculateDotProduct(planes->GetCurrent(), max.x, min.y, min.z) < 0.0f)
			return false;
		if (CalculateDotProduct(planes->GetCurrent(), min.x, max.y, min.z) < 0.0f)
			return false;
		if (CalculateDotProduct(planes->GetCurrent(), max.x, max.y, min.z) < 0.0f)
			return false;
		if (CalculateDotProduct(planes->GetCurrent(), min.x, min.y, max.z) < 0.0f)
			return false;
		if (CalculateDotProduct(planes->GetCurrent(), max.x, min.y, max.z) < 0.0f)
			return false;
		if (CalculateDotProduct(planes->GetCurrent(), min.x, max.y, max.z) < 0.0f)
			return false;
		if (CalculateDotProduct(planes->GetCurrent(), max.x, max.y, max.z) < 0.0f)
			return false;
	}
...
{
	D3DXVECTOR3 diffVector = translation1 - translation2;
	// Get the distance between the two spheres.
	float distanceBetween = D3DXVec3Length(&diffVector) - radiiSum;
